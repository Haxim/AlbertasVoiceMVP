...
export default async function ResetPasswordPage({
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-12">
      ...
      <p className="mt-2 text-ink/70">Enter a new password for your captain account.</p>
      {params?.message ? <p className="mt-4 rounded-md bg-white p-3 text-sm">{params.message}</p> : null}
      <form action={updatePassword} className="mt-6 space-y-4 rounded-lg border border-line bg-white p-5">
        ...
        <button className="focus-ring w-full rounded-md bg-spruce px-4 py-3 font-semibold text-white">Update password</button>
      </form>
    </main>
  );
}
